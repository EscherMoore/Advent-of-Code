...
int main(int argc, char *argv[]) {
  ...
  while (std::getline(file, line)) {
    boost::split(
        nums, line, boost::algorithm::is_any_of(" "), boost::token_compress_on
    );
    left_list.push_back(std::stoi(nums[0]));
    right_list.push_back(std::stoi(nums[1]));
  }
  ...
  int total_disance = 0;
  for (int idx = 0; idx != left_list.size(); ++idx) {
    total_disance += std::abs(left_list[idx] - right_list[idx]);
  }
  std::cout << "Total Distance: " << total_disance << std::endl;
}
