...
char findLast(std::string &line) {
  for (auto ch = line.end(); ch >= line.begin(); --ch) {
    if (isdigit(*ch)) {
      return *ch;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (calibrationDocument) {
    ...
    while (calibrationDocument >> calibrationValue) { 
      char first = findFirst(calibrationValue);
      char last = findLast(calibrationValue);

      if (first != -1 && last != -1) {
        std::string joinedInts;
        joinedInts += first;
        joinedInts += last;
        sum += std::stoi(joinedInts); 
      } else {
        std::cout << "Something went wrong with: " << calibrationValue
                  << std::endl;
      }
    }
  } else {
