...
using matchCnt = int;
using cardCnt = int;
using copiesDerivedFromCardNum = std::vector<int>;

std::map<cardNum, matchCnt> cardMatchCount;
std::map<cardNum, cardCnt> totalScratchCards;
std::map<cardNum, copiesDerivedFromCardNum> copiesTable;

void search(copiesDerivedFromCardNum cardNumbers);
void getCardMatchCount(std::ifstream &scratchcards);
void buildCopiesTable();

int main(int argc, char *argv[]) {
  std::ifstream scratchcards(argv[1]);

  if (!scratchcards) {
    std::cout << "Failed to open the scratch cards table!" << std::endl;
  }
  
  getCardMatchCount(scratchcards); 
  buildCopiesTable();

  for (std::pair<cardNum, matchCnt> scratchCard : cardMatchCount) {
    totalScratchCards[scratchCard.first] = 1; // count the original card 
  }

  for (std::pair<cardNum, copiesDerivedFromCardNum> scratchCard : copiesTable) {
    search(scratchCard.second);
  }

  int sum = 0;
  for (std::pair<cardNum, matchCnt> scratchCard : totalScratchCards) {
    sum += scratchCard.second;
  }
  std::cout << "Answer: " << sum << std::endl;
}

void search(copiesDerivedFromCardNum copies) {
  for (cardNum cardNum : copies) {
    totalScratchCards[cardNum] += 1;
    search(copiesTable[cardNum]);
  }
}
...
void getCardMatchCount(std::ifstream &scratchcards) {
  ...
  while (getline(scratchcards, scratchcard)) {
    int gameStartIdx = scratchcard.find(":");
    scratchcard.erase(0, gameStartIdx+1);

    int endOfMyCards = scratchcard.find("|");
    std::string contenders = scratchcard.substr(0, endOfMyCards);
    std::string winners = scratchcard.substr(
      endOfMyCards+1, scratchcard.length()
    );

    std::map<std::string, std::string> winningNumbers;
    std::string winningNumber;
    for (auto digit = winners.begin(); digit != winners.end(); ++digit) {
      if (*digit == ' ') {
    ...
    int matchCnt = 0;
    for (auto digit = contenders.begin(); digit != contenders.end(); ++digit) {
      if (*digit == ' ') {
        if (!winningNumbers[contender].empty()) {
          matchCnt += 1;
        }
        contender = "";
      } else {
    ...
    cardNum += 1;
    cardMatchCount[cardNum] = matchCnt;
  }
}

void buildCopiesTable() {
  for (std::pair<cardNum, matchCnt> scratchCard : cardMatchCount) {
    for (cardNum copy = scratchCard.first+1; 
        copy <= scratchCard.first + scratchCard.second; 
        ++copy
    ) {
      copiesTable[scratchCard.first].push_back(copy);
    }
  }
}
