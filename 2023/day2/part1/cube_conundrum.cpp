...
int main(int argc, char *argv[]) {
  ...
  if (games) {
    ...
    while(getline(games, game)) {
      ...
      while (game.length()) {
        size_t endOfRoundIdx = game.find(";");
        std::string round = game.substr(0, endOfRoundIdx);

        for (auto color : colors) {
          size_t colorStartingIdx = round.find(color.first);
          if (colorStartingIdx != std::string::npos) {
            std::string roundWithoutColor = round.substr(0, colorStartingIdx);
            std::string cubeCountStr;
            for (auto num = roundWithoutColor.rbegin(); 
                *num != ',' && *num != ';' && *num != ':' &&
                isdigit(*num) && num != roundWithoutColor.rend();
              ++num) 
            {
              cubeCountStr = *num + cubeCountStr;
            }
            int cubeCount = stoi(cubeCountStr);
            if (cubeCount > color.second) {
              possible = false;
            }
          }
        }
        if (endOfRoundIdx == std::string::npos) {
          if (possible) {
            validGameIdSum += gameId;
          }
          game = "";
        } else {
          game.erase(0, endOfRoundIdx + 1);
        }
      }
    }
    std::cout << "Answer: " << validGameIdSum << std::endl;
  } else {
    std::cout << "Failed to open the game record file." << std::endl;
  }
}
