...
int search(const std::string &mapValue, int direction = 0, int steps = 0);
...
int main(int argc, char *argv[]) {
  ...
  int steps = search("AAA");
  std::cout << "Answer: " << steps << std::endl;
}

int search(const std::string &mapValue, int direction, int steps) {
  std::vector<std::string> &currentMap = map[mapValue];
  int left = 0, right = 1;

  switch (directions[direction]) {
    case 'L':
      return search(currentMap[left], direction + 1, steps + 1);
    case 'R':
      return search(currentMap[right], direction + 1, steps + 1);
  }

  if (mapValue != "ZZZ") {
    return search(mapValue, 0, steps);
  }
  return steps;
}
