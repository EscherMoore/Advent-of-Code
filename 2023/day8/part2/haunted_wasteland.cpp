...
bool isParenthesisOrComma(char ch);
long long search(
    const std::string& mapValue, int direction = 0, int steps = 0);
...
int main(int argc, char *argv[]) {
  ...
  while (getline(input, line)) {
    ...
    std::string value, equalSign, left, right;
    std::stringstream ss(line);
    while(ss >> value >> equalSign >> left >> right) {
      left.erase(std::remove_if(
        left.begin(), left.end(), &isParenthesisOrComma), left.end());
      right.erase(std::remove_if(
        right.begin(), right.end(), &isParenthesisOrComma), right.end());

      map[value].push_back(left);
      map[value].push_back(right);
    }
  }

  std::vector<long long> cycleLengths;
  for (auto value : map) {
    std::string mapValue = value.first;
    if (mapValue[2] == 'A') {
      long long cycleLength = search(mapValue);
      cycleLengths.push_back(cycleLength);
    }
  }

  long long syncPoint = cycleLengths[0];
  for (long long cycleLength = 0;
      cycleLength != cycleLengths.size();
      ++cycleLength
    ) {
    syncPoint = lcm(syncPoint, cycleLengths[cycleLength]);
  }
  std::cout << "Answer: " << syncPoint << std::endl;
}

long long search(const std::string& mapValue, int direction, int steps) {
  if (mapValue[2] == 'Z') {
    return steps;
  }

  int left = 0, right = 1;
  std::vector<std::string> &currentMap = map[mapValue];

  int nextDirection = (direction + 1) % directions.size();
  
  switch (directions[direction]) {
    case 'L':
      return search(currentMap[left], nextDirection, steps + 1);
    case 'R':
      return search(currentMap[right], nextDirection, steps + 1);
  }
  return steps;
}
