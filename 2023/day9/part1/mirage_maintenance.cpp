...
int main(int argc, char *argv[]) {
  ...
  for (auto historyOfValue = history.begin();
    ...
    for (auto currentSequenceOfDifferences = sequencesOfDifferences.rbegin();
      ...
      int valueLeft = currentSequenceOfDifferences->back(), valueBelow = 0;
      if (currentSequenceOfDifferences != sequencesOfDifferences.rbegin()) {
        valueBelow = (currentSequenceOfDifferences - 1)->back(); 
      }
      currentSequenceOfDifferences->push_back(valueBelow + valueLeft);
    }
    sum += sequencesOfDifferences.front().back();
  }
