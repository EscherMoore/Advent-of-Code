...
bool allZeros(const std::vector<int> &sequence);
...
int main(int argc, char *argv[]) {
  ...
  while(getline(input, line)) {
    std::vector<int> historyOfValue;

    std::string value;
    std::stringstream ss(line);
    while (ss >> value) {
      historyOfValue.push_back(stoi(value));
    }
    history.push_back(historyOfValue);
  }

  int sum = 0;
  for (auto historyOfValue = history.begin();
    historyOfValue != history.end(); 
    ++historyOfValue
  ) {
    std::vector<std::vector<int>> sequencesOfDifferences = { *historyOfValue };

    while (true) {
      std::vector<int> nextSequenceOfDifferences;
      std::vector<int> currentSequenceOfDifferences = sequencesOfDifferences.back();

      for (auto value = currentSequenceOfDifferences.begin();
        value != currentSequenceOfDifferences.end() - 1;
        ++value
      ) {
        int difference = *(value + 1) - *value;
        nextSequenceOfDifferences.push_back(difference);
      }

      if (allZeros(nextSequenceOfDifferences)) {
        break;
      }
      sequencesOfDifferences.push_back(nextSequenceOfDifferences);
    }
    
    for (auto currentSequenceOfDifferences = sequencesOfDifferences.rbegin();
      currentSequenceOfDifferences != sequencesOfDifferences.rend();
      ++currentSequenceOfDifferences
    ) {
      int valueRight = currentSequenceOfDifferences->front(), valueBelow = 0;
      if (currentSequenceOfDifferences != sequencesOfDifferences.rbegin()) {
        valueBelow = (currentSequenceOfDifferences - 1)->front(); 
      }
      currentSequenceOfDifferences->insert(
        currentSequenceOfDifferences->begin(), valueRight - valueBelow);
    }
    sum += sequencesOfDifferences.front().front();
  }
  std::cout << "Answer: " << sum << std::endl;
}

bool allZeros(const std::vector<int> &sequenceOfDifferences) {
  for (int difference : sequenceOfDifferences) {
    if (difference != 0) {
      return false; 
    }
  }
