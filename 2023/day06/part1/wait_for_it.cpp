...
int holdButton(int msToHold, int msOfTime);
...
int main(int argc, char *argv[]) {
  ...
  std::vector<int> raceDurations = parseNumbers(time, "Time:");

  std::string distance;
  getline(raceDocument, distance);
  std::vector<int> recordDistances = parseNumbers(distance, "Distance:");

  int product = 1;
  for (int idx = 0; idx != recordDistances.size(); ++idx) {
    int waysToBeat = 0;
    for (int millisecondsToHold = 1;
        millisecondsToHold != raceDurations[idx];
        ++millisecondsToHold
    ) {
      int distanceTraveled = holdButton(millisecondsToHold, raceDurations[idx]);
      if (distanceTraveled > recordDistances[idx]) {
        waysToBeat += 1;
      }
    }
    product *= waysToBeat;
...
std::vector<int> parseNumbers(
  ...
  while (ss >> number) {
    ...
    parsedNumbers.push_back(stoi(number));
  }
