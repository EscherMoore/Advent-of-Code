...
long long parseNumbers(
  std::string &numbers, const std::string &ignoreValue
);
long long holdButton(long long msToHold, long long msOfTime);

int main(int argc, char *argv[]) {
  std::ifstream raceDocument(argv[1]);

  if (!raceDocument) {
    std::cout << "Failed to open the race document!" << std::endl;
    return 1;
  }

  std::string time;
  getline(raceDocument, time);
  long long raceDuration = parseNumbers(time, "Time:");

  std::string distance;
  getline(raceDocument, distance);
  long long recordDistance = parseNumbers(distance, "Distance:");
  
  long long waysToBeat = 0;
  for (long long millisecondsToHold = 1; millisecondsToHold != raceDuration; ++millisecondsToHold) {
    long long distanceTraveled = holdButton(millisecondsToHold, raceDuration);
    if (distanceTraveled > recordDistance) {
      waysToBeat += 1;
    }
  }
  std::cout << "Answer: " << waysToBeat << std::endl;
}
...
long long parseNumbers(
  ...
  return stoll(parsedNumber);
}

long long holdButton(long long msToHold, long long msOfTime) {
  long long &msHeld = msToHold;
  long long msLeft = msOfTime - msToHold;
  return (msLeft * msHeld);
}
