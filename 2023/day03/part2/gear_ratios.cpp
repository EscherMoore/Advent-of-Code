...
void handleAdjacentGears(const int row, const int col, const int len, int partNumber);
bool isGear(const int row, const int col, int partNumber);
bool isValidCell(const int row, const int col);

std::vector<std::vector<char>> matrix;
std::map<int, std::map<int, std::vector<int>>> gears;
int sum = 0;
...
int main(int argc, char *argv[]) {
  ...
  if (!engineSchematic) {
    std::cout << "Failed to open engine schematic." << std::endl;
    return -1;
  }
  ...
  for (int currentRow = 0; currentRow <= matrix.size() - 1; ++currentRow) {
        ...
        currentCol <= matrix[currentRow].size() - 1; ++currentCol) 
    {
      ...
      if (isdigit(ch)) {
        ...
        handleAdjacentGears(currentRow, currentCol - len, len, number);
        digit = "";
      }
    }
    if (digit != "") {
      int number = stoi(digit), len = digit.length();
      handleAdjacentGears(currentRow, (matrix[currentRow].size() - 1) - len, len, number);
      digit = "";
    }
  }
  ...
  for (int currentRow = 0; currentRow <= matrix.size() - 1; ++currentRow) {
    for (int currentCol = 0; 
        currentCol <= matrix[currentRow].size() - 1; ++currentCol) 
    {
      const std::vector<int> &partNumbers = gears[currentRow][currentCol];
...
void handleAdjacentGears(const int row, const int col, const int len, int partNumber) {
  for (int currentCol = col; currentCol < col + len; ++currentCol) {
    if (
      isGear(row, currentCol - 1, partNumber) || // left
      isGear(row, currentCol + 1, partNumber) || // right
      isGear(row - 1, currentCol, partNumber) || // top
      isGear(row - 1, currentCol - 1, partNumber) || // top left
      isGear(row - 1, currentCol + 1, partNumber) || // top right
      isGear(row + 1, currentCol, partNumber) || // bottom
      isGear(row + 1, currentCol - 1, partNumber) || // bottom left
      isGear(row + 1, currentCol + 1, partNumber) // bottom right
    ) {
      return;
    }
  }
...
bool isGear(const int row, const int col, int partNumber) {
    ...
    if (ch == '*') {
      auto &partNumbers = gears[row][col];
